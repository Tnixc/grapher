...
class Grapher {
    ...
    updateFunction(func) {
        try {
            ...
            this.updateFunctionInfo(func, `<span style="color: #ff4444;">Error: ${error.message}</span>`);
            func.parsed = null;
            func.asymptotes = [];
            func.holes = [];
        }
    ...
    detectAsymptotesAndHoles(func) {
        ...
        if (rightY !== null && Math.abs(rightY) < 1000 && 
    ...
    drawFunction(func) {
        ...
        for (let px = 0; px <= width; px += 0.5) {
            ...
            if (y !== null) {
                ...
                if (py >= -50 && py <= height + 50) { // Extended bounds for smoother curves
                    if (!pathStarted) {
                        ctx.moveTo(px, py);
                        pathStarted = true;
                    } else {
